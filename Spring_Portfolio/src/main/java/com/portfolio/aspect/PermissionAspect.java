...
public class PermissionAspect {
	...
	public Object checkPermission(ProceedingJoinPoint joinPoint, RequiresPermission requiresPermission) throws Throwable {
	 ...
	 // 先檢查 permissionBean 是否存在，避免 NullPointerException
	 if (permissionBean == null) {
		 throw new UnauthorizedException(MessageConstants.NO_PERMISSION_ERROR_MSG);
	 }

	 // 檢查是否具備所需權限
	 boolean allowed = hasPermission(permissionBean, requiresPermission.value());
	 // 這裡可以改為 LogUtils 或其他日誌框架記錄，暫時使用 System.out 作為簡易偵錯
	 System.out.println("Permission check for " + requiresPermission.value() + " => " + allowed);
	 if (!allowed) {
		 throw new UnauthorizedException(MessageConstants.NO_PERMISSION_ERROR_MSG);
	 }
